...
public class UpdateOrderUseCase(IOrderRepository orderRepo, IOrderItemRepository orderItemRepo)
    : IUpdateOrderUseCase
{
    public async Task<Order> Execute(UpdateOrderDto orderDto)
    {
        ...
        var itemsToDelete = orderFromDb
            .OrderItems.Where(dbItem => !dtoItemIds.Contains(dbItem.Id))
            .ToList();
        ...
        {
            var existingItem = orderFromDb.OrderItems.FirstOrDefault(dbItem =>
                dbItem.Id == dtoItem.Id
            );
            ...
            if (existingItem == null)
            {
                ...
                var newItem = new OrderItem
                {
                    ...
                    ProductId =
                        dtoItem.ProductId
                        ?? throw new Exception("ProductId is required for new items"),
                    Quantity =
                        dtoItem.Quantity
                        ?? throw new Exception("Quantity is required for new items"),
